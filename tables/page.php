...
$query = $wpdb->get_row("SELECT count(*) as total FROM $table_name WHERE id > 0 $table_criteria ".$_GET['selection_criteria']." $search_criteria", OBJECT);
$n = (int) $query->total;
...
$items = $wpdb->get_results("SELECT * FROM $table_name WHERE id > 0 $table_criteria ".$_GET['selection_criteria']." $search_criteria ORDER BY ".$_GET['orderby']." ".strtoupper($_GET['order'])." LIMIT $start, $limit", OBJECT); ?>
...
<p class="search-box" style="float: right;"><?php _e('Search by', 'affiliation-manager'); ?> <select name="searchby" id="searchby">
...
switch ($_GET['page']) {
case 'affiliation-manager-affiliates': $row_actions = '<div class="row-actions" style="margin-top: 2em; position: absolute; width: 1000%;"><span class="edit">
...
case 'affiliation-manager-clicks': $row_actions = '<div class="row-actions" style="margin-top: 2em; position: absolute; width: 1000%;"><span class="delete">
<a href="admin.php?page=affiliation-manager-clicks&amp;id='.$item->id.'&amp;action=delete">'.__('Delete').'</a></span></div>'; break;
case 'affiliation-manager-commissions': if (function_exists('commerce_manager_admin_menu')) {
...
<?php tablenav_pages($n, $max_paged, 'bottom'); ?>
<div class="alignleft actions">
