...
$results = $wpdb->query("DELETE FROM ".$wpdb->prefix."audiobooks_authors_and_narrators_authors WHERE id = ".$_GET['id']);
$result = $wpdb->get_row("SELECT id FROM ".$wpdb->prefix."audiobooks_authors_and_narrators_authors ORDER BY id DESC LIMIT 1", OBJECT);
if (!$result) { $results = $wpdb->query("ALTER TABLE ".$wpdb->prefix."audiobooks_authors_and_narrators_authors AUTO_INCREMENT = 1"); }
elseif ($result->id < $_GET['id']) {
$results = $wpdb->query("ALTER TABLE ".$wpdb->prefix."audiobooks_authors_and_narrators_authors AUTO_INCREMENT = ".($result->id + 1)); } } } ?>
<div class="wrap">
<div id="poststuff" style="padding-top: 0;">
<?php audiobooks_authors_and_narrators_pages_top($back_office_options); ?>
<?php if (isset($_POST['submit'])) {
echo '<div class="updated"><p><strong>'.__('Author deleted.', 'audiobooks-authors-and-narrators').'</strong></p></div>
<script type="text/javascript">setTimeout(\'window.location = "admin.php?page=audiobooks-authors-and-narrators-authors"\', 2000);</script>'; } ?>
...
<p><strong style="color: #c00000;"><?php _e('Do you really want to permanently delete this author?', 'audiobooks-authors-and-narrators'); ?></strong> 
...
foreach ($tables['authors'] as $key => $value) { if (!isset($_POST[$key])) { $_POST[$key] = ''; } }
if ((isset($_POST['submit'])) && (check_admin_referer($_GET['page']))) {
if (!audiobooks_authors_and_narrators_user_can($back_office_options, 'manage')) { $_POST = array(); $error = __('You don\'t have sufficient permissions.', 'audiobooks-authors-and-narrators'); }
else {
...
if (isset($_GET['id'])) {
$author_data = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."audiobooks_authors_and_narrators_authors WHERE id = ".$_GET['id'], OBJECT);
if ($author_data) {
$GLOBALS['author_data'] = (array) $author_data;
foreach ($author_data as $key => $value) { if ((!isset($_POST[$key])) || (!isset($_POST[$key.'_error']))) { $_POST[$key] = $value; } } }
elseif (!headers_sent()) { header('Location: admin.php?page=audiobooks-authors-and-narrators-authors'); exit(); }
else { echo '<script type="text/javascript">window.location = "admin.php?page=audiobooks-authors-and-narrators-authors";</script>'; } }
...
$undisplayed_modules = (array) $back_office_options['author_page_undisplayed_modules']; ?>

<div class="wrap">
<div id="poststuff" style="padding-top: 0;">
<?php audiobooks_authors_and_narrators_pages_top($back_office_options); ?>
<?php if ((isset($updated)) && ($updated)) {
echo '<div class="updated"><p><strong>'.(isset($_GET['id']) ? __('Author updated.', 'audiobooks-authors-and-narrators') : __('Author saved.', 'audiobooks-authors-and-narrators')).'</strong></p></div>
'.(isset($_GET['id']) ? '' : '<script type="text/javascript">setTimeout(\'window.location = "admin.php?page=audiobooks-authors-and-narrators-authors"\', 2000);</script>'); } ?>
<form method="post" action="<?php echo esc_attr($_SERVER['REQUEST_URI']); ?>">
<?php wp_nonce_field($_GET['page']); ?>
<?php audiobooks_authors_and_narrators_pages_menu($back_office_options); ?>
<div class="clear"></div>
...
<h3 style="font-size: 1.25em;" id="personal-informations"><strong><?php echo $modules['author']['personal-informations']['name']; ?></strong></h3>
<div class="inside">
...
<a style="text-decoration: none;" href="admin.php?page=audiobooks-authors-and-narrators-author&amp;id='.$_GET['id'].'&amp;action=delete" class="delete">'.__('Delete', 'audiobooks-authors-and-narrators').'</a></td></tr>'; } ?>
...
<?php $url = htmlspecialchars(author_data(array(0 => 'audiobooks_url', 'part' => 1, 'id' => (isset($_GET['id']) ? $_GET['id'] : 0)))); if ($url != '') { ?><a style="vertical-align: 25%;" href="<?php echo $url; ?>"><?php _e('Link', 'audiobooks-authors-and-narrators'); ?></a><?php } ?></td></tr>
...
<?php $url = htmlspecialchars(author_data(array(0 => 'profile_url', 'part' => 1, 'id' => (isset($_GET['id']) ? $_GET['id'] : 0)))); if ($url != '') { ?><a style="vertical-align: 25%;" href="<?php echo $url; ?>"><?php _e('Link', 'audiobooks-authors-and-narrators'); ?></a><?php } ?></td></tr>
...
<p class="submit"><input type="submit" class="button-primary" name="submit" id="submit" value="<?php (isset($_GET['id']) ? _e('Save Changes', 'audiobooks-authors-and-narrators') : _e('Save Author', 'audiobooks-authors-and-narrators')); ?>" /></p>
</form>
</div>
</div>
...
<?php foreach ($modules['author'] as $key => $value) {
...
<?php $fields = array();
foreach ($tables['authors'] as $key => $value) { $fields[] = $key; }
