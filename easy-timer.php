...
define('EASY_TIMER_OPTIONS_ELAPSED_DEFAULT_SHORTCODE', '[elapsed-'.substr($easy_timer_options['default_shortcode'], 1));
define('EASY_TIMER_OPTIONS_REMAINING_DEFAULT_SHORTCODE', '[remaining-'.substr($easy_timer_options['default_shortcode'], 1));
...
$content = str_ireplace('[elapsed-timer]', EASY_TIMER_OPTIONS_ELAPSED_DEFAULT_SHORTCODE, $content);
...
$content = str_ireplace('[remaining-timer]', EASY_TIMER_OPTIONS_REMAINING_DEFAULT_SHORTCODE, $content);
...
if ($i < $n) { $remaining_S = $T[$i] - $time; }
else { $i = $n - 1; $remaining_S = 0; }
...
if ($way == 'up') { $content[$k] = timer_replace($S[$k], $T[$k], 'up', $content[$k]); }
if ($way == 'down') { $content[$k] = timer_replace($remaining_S, $T[$i], 'down', $content[$k]); }
$content[$k] = elapsed_timer_replace($S[$k], $T[$k], $content[$k]);
$content[$k] = remaining_timer_replace($remaining_S, $T[$i], $content[$k]);
...
'.__('The <code>[elapsed-timer]</code> shortcode is equivalent to', 'easy-timer').' <code>[elapsed-'.substr($easy_timer_options['default_shortcode'], 1).'</code>.<br />
'.__('The <code>[remaining-timer]</code> shortcode is equivalent to', 'easy-timer').' <code>[remaining-'.substr($easy_timer_options['default_shortcode'], 1).'</code>.</p>
