...
$sql = "CREATE TABLE ".$wpdb->prefix."audiobooks_authors_and_narrators_".$table_slug." (".$list."
PRIMARY KEY  (id)) $charset_collate;"; dbDelta($sql);
foreach ($table as $key => $value) {
if (isset($value['constraint'])) {
$results = $wpdb->query("ALTER TABLE ".$wpdb->prefix."audiobooks_authors_and_narrators_".$table_slug." ADD ".$value['constraint']." (".$key.")"); }
if (isset($value['default'])) {
$results = $wpdb->query("UPDATE ".$wpdb->prefix."audiobooks_authors_and_narrators_".$table_slug." SET ".$key." = '".$value['default']."' WHERE ".$key." = ''"); } } }
...
$_key = ($key == '' ? '' : '_'.$key);
if (is_array($value)) {
$options = (array) get_option('audiobooks_authors_and_narrators'.$_key);
...
if ($options != $current_options) { update_option('audiobooks_authors_and_narrators'.$_key, $options); } }
else { add_option(substr('audiobooks_authors_and_narrators'.$_key, 0, 64), $value); } }

if (function_exists('date_default_timezone_set')) { date_default_timezone_set('UTC'); }
$cron = (array) get_option('audiobooks_authors_and_narrators_cron');
if ((!isset($cron['previous_installation'])) || ($cron['previous_installation']['version'] != AUDIOBOOKS_AUTHORS_AND_NARRATORS_VERSION)) {
$cron['previous_installation'] = array('version' => AUDIOBOOKS_AUTHORS_AND_NARRATORS_VERSION, 'number' => 1); }
