...
$atts['data'] = $datas[$i];
$data = $data + audiobooks_authors_and_narrators_counter($atts, '[total-number]'); } }
else {
$data = str_replace('_', '-', format_nice_name($data));
switch ($data) {
case 'authors': $table = $wpdb->prefix.'audiobooks_authors_and_narrators_authors'; $field = ''; break;
case 'narrators': $table = $wpdb->prefix.'audiobooks_authors_and_narrators_narrators'; $field = ''; break;
default: $table = $wpdb->prefix.'audiobooks_authors_and_narrators_authors'; $field = ''; }
...
$data_key = "audiobooks_authors_and_narrators_".$date_criteria."_".$status_criteria."_".$data;
...
if (isset($GLOBALS['audiobooks_authors_and_narrators_'.$_tag])) { $original['audiobooks_authors_and_narrators_'.$_tag] = $GLOBALS['audiobooks_authors_and_narrators_'.$_tag]; }
remove_shortcode($tag); add_shortcode($tag, create_function('$atts', '$atts["data"] = "'.$tag.'"; return audiobooks_authors_and_narrators_counter_tag($atts);')); }

$GLOBALS['audiobooks_authors_and_narrators_limit'] = $limit[$i];
$GLOBALS['audiobooks_authors_and_narrators_number'] = $data - $limit[$k];
$GLOBALS['audiobooks_authors_and_narrators_remaining_number'] = $remaining_number;
$GLOBALS['audiobooks_authors_and_narrators_total_limit'] = $limit[$n - 1];
$GLOBALS['audiobooks_authors_and_narrators_total_number'] = $data;
$GLOBALS['audiobooks_authors_and_narrators_total_remaining_number'] = $total_remaining_number;

$content = (isset($content[$k]) ? do_shortcode($content[$k]) : '');
$content = audiobooks_authors_and_narrators_filter_data($filter, $content);

foreach ($tags as $tag) {
$_tag = str_replace('-', '_', format_nice_name($tag));
if (isset($original['audiobooks_authors_and_narrators_'.$_tag])) { $GLOBALS['audiobooks_authors_and_narrators_'.$_tag] = $original['audiobooks_authors_and_narrators_'.$_tag]; }
