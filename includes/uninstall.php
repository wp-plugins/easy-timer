...
deactivate_plugins(AUDIOBOOKS_AUTHORS_AND_NARRATORS_FOLDER.'/audiobooks-authors-and-narrators.php');
if ($for == 'network') {
...
if ($plugin != AUDIOBOOKS_AUTHORS_AND_NARRATORS_FOLDER.'/audiobooks-authors-and-narrators.php') { $new_active_plugins[] = $plugin; } }
...
$results = $wpdb->query("DROP TABLE ".$blog_prefix.'audiobooks_authors_and_narrators_'.$table_slug); } }
...
$_key = ($key == '' ? '' : '_'.$key);
delete_option(substr('audiobooks_authors_and_narrators'.$_key, 0, 64)); } }
switch_to_blog($original_blog_id); }
else {
foreach ($initial_options as $key => $value) {
$_key = ($key == '' ? '' : '_'.$key);
