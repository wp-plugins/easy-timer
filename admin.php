<?php if (strstr($_GET['page'], 'affiliation-manager')) { wp_enqueue_script('dashboard'); }

function affiliation_manager_affiliate_page() { include 'affiliate-page.php'; }
function affiliation_manager_back_office_page() { include 'back-office-page.php'; }
function affiliation_manager_options_page() { include 'options-page.php'; }
function affiliation_manager_payment_page() { include 'payment-page.php'; }
...
add_menu_page('Affiliation Manager', __('Affiliation', 'affiliation-manager'), 'manage_options', 'affiliation-manager', 'affiliation_manager_options_page', '');
foreach ($admin_pages as $key => $value) {
add_submenu_page('affiliation-manager', $value['page_title'], $value['menu_title'], 'manage_options', 'affiliation-manager'.($key == '' ? '' : '-'.str_replace('_', '-', $key)), $value['function']); } }

add_action('admin_menu', 'affiliation_manager_admin_menu');


function affiliation_manager_pages_date_picker($start_date, $end_date) {
echo '<p style="margin: 0 0 1em 0;"><label><strong>'.__('Start', 'affiliation-manager').'</strong>
...
(((strstr($_GET['page'], 'commissions')) && (!strstr($_GET['page'], 'prospects')) && (!strstr($_GET['page'], 'recurring'))) ? '-orders' : '').
...
else { echo '<label'.((($page_slug != 'affiliate') || (!isset($_GET['id'])) || (!in_array($key, array('registration-confirmation-email', 'registration-notification-email', 'autoresponders', 'membership', 'custom-instructions')))) ? '' : ' style="display: none;"').'><input type="checkbox" name="'.$name.'" id="'.$name.'" value="yes"'.(in_array($key, $page_undisplayed_modules) ? '' : ' checked="checked"').' /> '.$value['name'].'<br /></label>'; }
...
if (($page_slug != 'affiliate') || (!isset($_GET['id'])) || (!in_array($key, array('registration-confirmation-email', 'registration-notification-email', 'autoresponders', 'membership', 'custom-instructions')))) {
...
include 'tables.php';
unset($tables['commissions']); unset($tables['prospects_commissions']); unset($tables['recurring_commissions']);
