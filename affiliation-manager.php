...
$local_affiliate_data = $wpdb->get_row("SELECT * FROM $affiliates_table_name WHERE id = '$id'", OBJECT);
$data = $local_affiliate_data->$field; }
...
<tr><th>'.__('Date', 'affiliation-manager').'</th><th>'.__('Hour', 'affiliation-manager').'</th><th>'.__('Referring URL', 'affiliation-manager').'</th></tr>';
foreach ($clicks as $click) {
$content .= '<tr><td>'.substr($click->date, 0, 10).'</td>
<td>'.substr($click->date, -8).'</td>
<td>'.str_replace('&', '&amp;', $click->referring_url).'</td></tr>'; }
$content .= '</table>'; }
else { $content .= '<p>'.__('No click', 'affiliation-manager').'</p>'; } }
...
if ($orders) {		
$content .= '<table style="width: 100%;">
<tr><th>'.__('Date', 'affiliation-manager').'</th><th>'.__('Hour', 'affiliation-manager').'</th><th>'.__('Amount', 'affiliation-manager').'</th><th>'.__('Paid', 'affiliation-manager').'</th></tr>';
foreach ($orders as $order) {
$content .= '<tr><td>'.substr($order->date, 0, 10).'</td>
<td>'.substr($order->date, -8).'</td>
<td>'.$order->commission_amount.' '.$currency_code.'</td>
<td>'.__($order->commission_paid, 'affiliation-manager').'</td></tr>'; }
$content .= '</table>'; }
else { $content .= '<p>'.__('No commission', 'affiliation-manager').'</p>'; } }
...
$result = $wpdb->get_results("SELECT login FROM $affiliates_table_name WHERE login='$login' AND password = '".md5($password)."'", OBJECT);
if (!$result) { $error .= __('Invalid login or password', 'affiliation-manager'); }
if ($error == '') {
$_SESSION['a_login'] = $login;
if (isset($_POST['a_remember'])) { setcookie('a_login', $login.md5($login.AUTH_KEY), time() + 90*86400, '/'); }
affiliation_instructions(); } }

if ($error != '') { $content .= '<p class="error">'.$error.'</p>'; }

$content .= '
<form method="post" action="'.$_SERVER['REQUEST_URI'].'" onsubmit="return validate_affiliation_login_form(this);">
...
<p style="margin: 0.75em; text-align: center;"><input type="checkbox" name="a_remember" id="a_remember" value="yes" /> <label for="a_remember">'.__('Remember me', 'affiliation-manager').'</label></p>
...
if (!$result) { $error .= __('This email address does not match an affiliate account.', 'affiliation-manager'); }
if ($error == '') {
$password = mt_rand(10000000, 99999999);
$login = $result->login;
$results = $wpdb->query("UPDATE $affiliates_table_name SET password = '".md5($password)."' WHERE email_address='$email_address'");
...
$content .= '
<form style="text-align: center;" method="post" action="'.$_SERVER['REQUEST_URI'].'" onsubmit="return validate_affiliation_password_reset_form(this);">
...
$results = $wpdb->query("UPDATE $affiliates_table_name SET
	...
	street = '".$_POST['a_street']."',
	postcode = '".$_POST['a_postcode']."',
	town = '".$_POST['a_town']."',
	country = '".$_POST['a_country']."',
	phone_number = '".$_POST['a_phone_number']."' WHERE login = '".$_SESSION['a_login']."'");

if ($password != '') { $results = $wpdb->query("UPDATE $affiliates_table_name SET password = '".md5($password)."' WHERE login = '".$_SESSION['a_login']."'"); }

$affiliate_data = $wpdb->get_row("SELECT * FROM $affiliates_table_name WHERE login = '".$_SESSION['a_login']."'", OBJECT);
$content .= '<p class="valid">'.__('Your profile has been changed successfully.', 'affiliation-manager').'</p>'; } }

if ($error != '') { $content .= '<p class="error">'.$error.'</p>'; }

$content .= '
<form method="post" action="'.$_SERVER['REQUEST_URI'].'" onsubmit="return validate_affiliation_profile_form(this);">
<table style="width: 100%;">
...
<tr class="a-company"><td><strong><label for="a_company">'.__('Company', 'affiliation-manager').':</label></strong></td>
<td><input type="text" name="a_company" id="a_company" size="30" value="'.$affiliate_data->company.'" /></td></tr>
<tr class="a-website"><td><strong><label for="a_website">'.__('Website', 'affiliation-manager').':</label></strong></td>
<td><input type="text" name="a_website" id="a_website" size="30" value="'.$affiliate_data->website.'" /></td></tr>
<tr class="a-street"><td><strong><label for="a_street">'.__('Street', 'affiliation-manager').':</label></strong></td>
<td><input type="text" name="a_street" id="a_street" size="30" value="'.$affiliate_data->street.'" /></td></tr>
...
$content .= '
<form method="post" action="'.$_SERVER['REQUEST_URI'].'" onsubmit="return validate_affiliation_registration_form(this);">
<table style="width: 100%;">
...
<span class="error" id="a_paypal_email_address_error"></span></td></tr>
<tr class="a-company"><td><strong><label for="a_company">'.__('Company', 'affiliation-manager').':</label></strong></td>
<td><input type="text" name="a_company" id="a_company" size="30" value="'.$_POST['a_company'].'" /></td></tr>
<tr class="a-website"><td><strong><label for="a_website">'.__('Website', 'affiliation-manager').':</label></strong></td>
<td><input type="text" name="a_website" id="a_website" size="30" value="'.$_POST['a_website'].'" /></td></tr>
<tr class="a-street"><td><strong><label for="a_street">'.__('Street', 'affiliation-manager').':</label></strong></td>
<td><input type="text" name="a_street" id="a_street" size="30" value="'.$_POST['a_street'].'" /></td></tr>
<tr class="a-postcode"><td><strong><label for="a_postcode">'.__('Postcode', 'affiliation-manager').':</label></strong></td>
...
$login = substr($_COOKIE['a_login'], 0, -32);
if (substr($_COOKIE['a_login'], -32) == md5($login.AUTH_KEY)) { $_SESSION['a_login'] = $login; } }	
...
$row = $wpdb->get_row("SELECT SUM(commission_amount) AS total FROM $orders_table_name WHERE referrer = '".$_SESSION['a_login']."' AND commission_paid = 'yes' AND date BETWEEN '$start_date' AND '$end_date'", OBJECT);
$paid_commissions_total_amount = (double) $row->total;
$non_paid_commissions_total_amount = $commissions_total_amount - $paid_commissions_total_amount;
$currency_code = $commerce_manager_options['currency_code'];
return '
...
<tr><td><strong>'.__('Non paid commissions total amount', 'affiliation-manager').':</strong></td>
<td>'.$non_paid_commissions_total_amount.' '.$currency_code.'</td></tr>
</tbody></table>'; }
...
<script type="text/javascript">$(function(){ $('.date-pick').datePicker({startDate:'2011-01-01'}); });</script>
<?php }
...
global $click_data, $clicks_table_name, $wpdb;
if (is_string($atts)) { $field = $atts; } else { $field = $atts[0]; }
$field = str_replace('-', '_', affiliation_format_nice_name($field));
...
$local_click_data = $wpdb->get_row("SELECT * FROM $clicks_table_name WHERE id = '$id'", OBJECT);
$data = $local_click_data->$field; }
if ($data == '') { $data = $atts['default']; }
$data = str_replace('&', '&amp;', $data);
return $data; }
