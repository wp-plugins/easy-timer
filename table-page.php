<?php if ((strstr($_GET['page'], 'click')) && (isset($_GET['id'])) && ($_GET['action'] == 'delete')) {
...
<?php _e('columns starting from column', 'affiliation-manager'); ?> <input style="text-align: center;" type="text" name="start_column" id="start_column" size="2" value="<?php echo $start_column + 1; ?>" /> 
<input type="submit" class="button-secondary" name="submit" value="<?php _e('Update'); ?>" /><input type="hidden" name="submit" value="true" />
<input type="submit" class="button-secondary" name="reset_columns" value="<?php _e('Reset the columns order', 'affiliation-manager'); ?>" /><br />
<?php for ($i = 0; $i < $max_columns; $i++) {
echo '<label for="column'.$i.'">'.__('Column', 'affiliation-manager').' '.($i + 1).'</label> <select'.($i < 9 ? ' style="margin-left: 0.75em;"': '').' name="column'.$i.'" id="column'.$i.'">';
